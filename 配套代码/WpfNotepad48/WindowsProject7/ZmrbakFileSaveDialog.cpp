...
bool ZmrbakFileSaveDialog::ShowDialog(HWND hWND)
{
	...
	if (FAILED(pfd->SetFileName(L"*.txt"))) goto End;

	if (FAILED(pfdc->StartVisualGroup(CONTROL_GROUP, L"编码(&E):"))) goto End;
	if (FAILED(pfdc->AddComboBox(1))) goto End;
	if (FAILED(pfdc->AddControlItem(1, 0, L"ANSI"))) goto End;
	if (FAILED(pfdc->AddControlItem(1, 1, L"UTF-16 LE"))) goto End;
	if (FAILED(pfdc->AddControlItem(1, 2, L"UTF-16 BE"))) goto End;
	if (FAILED(pfdc->AddControlItem(1, 3, L"UTF-8"))) goto End;
	if (FAILED(pfdc->AddControlItem(1, 4, L"带有 BOM 的 UTF-8"))) goto End;
